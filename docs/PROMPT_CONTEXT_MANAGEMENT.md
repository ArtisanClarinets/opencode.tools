...
- `package.json`: Add dependencies `tree-sitter`, `tree-sitter-typescript`, and `faiss-node` (or a similar local vector store).
...
**Files to Create/Modify:**
- `src/cowork/persistence/migrations/xxxx_add_workspace_memory.sql` (NEW): Add a new table or JSONB column to track `filesReviewed`, `currentHypothesis`, and `pendingTasks` for a given session.
...
**Files to Create/Modify:**
- `package.json`: Add `swagger-parser` and `graphql` dependencies.
