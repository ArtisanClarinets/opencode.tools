...
const TECH_COMPLEXITY: Record<string, number> = {
    'react': 1.2,
    'next': 1.3,
    'vue': 1.2,
    'angular': 1.5,
...
export interface CostEstimate {
    phase: string;
    tasks: { name: string; hours: number; role: string }[];
    totalHours: number;
    cost: number;
}
 ...
 */
function calculateComplexity(stack: ProjectStack, structure: ProjectStructure): ProjectComplexity {
    let score = 0;
 ...
 */
function generateCostEstimate(projectName: string, complexity: ProjectComplexity): CostEstimate[] {
    const phases: CostEstimate[] = [];
 ...
 */
function calculateTimeline(phases: CostEstimate[]): { weeks: number; milestones: { name: string; week: number }[] } {
